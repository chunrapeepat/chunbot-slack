  ...
  .onPublish(async (message, ctx) => {
    ...
    if (channel.ok && channel.channel.name === 'feed-me') {
      ...
      if (event.type === 'message' && event.thread_ts !== undefined) {
        await createOrder(event.event_ts, event.thread_ts, user.profile.real_name, event.user, event.text);
      }
    }
    ...
    if (channel.ok && channel.channel.name === 'pay-me') {
      if (event.type === 'message' && event.thread_ts !== undefined && event.files !== undefined) {
        ...
        await updateMessage(event.channel, event.thread_ts, await getInvoiceMessage());
      };
    };
  });
